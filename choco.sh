...
ensure_local_venv() {
    ...
    if [ ! -x "$venv/bin/choco-server" ] || ! $as_user "$venv/bin/python" -c "import choco" 2>/dev/null; then
...
cmd_install() {
    ...
    # CLI: put the venv's `choco` console script (choco/cli.py, the
    # localhost JSON-API client) on PATH.  The daemon is `choco-server`.
    ln -sfn "$INSTALL_DIR/.venv/bin/choco" /usr/local/bin/choco
    ...
    if [ ! -f "$CONFIG_DIR/config.yaml" ]; then
        ...
        # The template's secret_key is a placeholder, and choco refuses to
        # start with one (a forged session cookie is a login bypass), so
        # seed a random key now rather than leave the first start to fail.
        if grep -qE '^\s*secret_key:\s*(change-me|dev-key-change-me)?\s*(#.*)?$' "$CONFIG_DIR/config.yaml"; then
            local key
            key=$(openssl rand -hex 32 2>/dev/null || python3 -c 'import secrets; print(secrets.token_hex(32))')
            sed -i "s|^\(\s*secret_key:\).*|\1 $key   # generated by choco.sh install|" "$CONFIG_DIR/config.yaml"
            echo "Generated a random server.secret_key."
        fi
    ...
    echo "  CLI:     choco nodes        (choco --help for the rest)"
}
...
cmd_uninstall() {
    ...
    # Application (and the CLI symlink, if it is ours)
    if [ "$(readlink /usr/local/bin/choco 2>/dev/null)" = "$INSTALL_DIR/.venv/bin/choco" ]; then
        rm -f /usr/local/bin/choco
    fi
    rm -rf "$INSTALL_DIR"
...
cmd_run() {
    ...
    exec sudo -u "$SUDO_USER" "$SCRIPT_DIR/.venv/bin/choco-server" "${@:-$SCRIPT_DIR/config.yaml}"
}
...
EOF
    ...
    echo "  CLI:      CHOCO_URL=http://127.0.0.1:$port $SCRIPT_DIR/.venv/bin/choco nodes"
    echo ""
    exec "$SCRIPT_DIR/.venv/bin/choco-server" "$dev_config"
}
...
cmd_help() {
    ...
    echo ""
    echo "Once installed, the 'choco' command (choco/cli.py) talks to the running"
    echo "daemon over its localhost API: choco --help"
}
