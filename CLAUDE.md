...
- **Service status strip & pages** — ``choco/services.py`` holds two small hardware monitors — ``FpgaMonitor`` (HTTP poll of fpga_master's ``/status`` + ``/get-frame0-time`` every 30s on its own gevent greenlet) and ``PsuMonitor`` (same shape; polls power_db's ``/status`` + ``/channel_states`` and decodes per-channel power via ``decode_out_bytes``, the same daisy-chain convention as ``jobs/bffs/sources/power.py``) — and a generic on-demand ``job_status(service_unit, state_file, stale_after_s)`` helper for the oneshot jobs (EOP, bffs, eigencal).  ``job_status`` combines two cheap signals without parsing any timestamps: ``systemctl show``'s ``Result`` (the only reliable "last run failed" signal; ``ExecMainExitTimestamp`` is tested for emptiness only, to detect never-run) and the job state file's mtime.  With ``stale_after_s`` set (EOP: 25 h — the job rewrites its state file on every successful daily run) an old mtime downgrades health to ``stale``; without it (bffs — state rewritten only when the bad-feed list *changes*; eigencal — daytime transits are silently skipped by design) the mtime is informational only.  All badges are surfaced by ``/partials/services``, rendered into ``_services_status.html``, and included from ``base.html`` above the nav for authenticated users (htmx polls every 30s).  Each badge links to a ``/service/<name>`` detail page (``service.html``; ``service_fpga.html`` / ``service_psu.html`` for the monitor-based badges — the PSU page renders the per-bus board/chip/channel grid whose cells are **toggle buttons** (confirm dialog → ``POST /service/psu/set`` → ``PsuMonitor.set_channel``, gated by ``psu.control``, audit-logged): power_db has no per-channel endpoint, so a toggle is a read-modify-write of the chip's whole OUT byte — the current byte is re-read immediately before the write and confirmed by a fresh read after (other writers exist: power_db's own CLI), with a mismatch reported as "state changed underneath us", never retried silently; the write side never depends on the read decode, and the byte framing in ``decode_out_bytes`` is verified against the live controller (a ch0 write to board 0 chip A moves exactly the last raw byte).  The FPGA page carries **Start/Stop controls**: confirm-dialog forms POSTing to ``/service/fpga/<action>``, gated by the ``fpga_master.control`` config flag (default true), audit-logged with the username; ``/start`` POSTs ``{}`` so fpga_master reuses its launch config and initializes in the background, ``/stop`` blocks remotely so ``FpgaMonitor.stop_master`` runs in a greenlet; the page's status block htmx-polls ``/partials/service-fpga`` every 10 s, whose ``poll_if_stale`` tightens the monitor cadence only while someone is watching.  A restart assigns a new frame0 — the page warns about it): common unit facts (``job_status`` detail plus the timer's next/last run via ``services.timer_status`` — systemd's own timestamp strings, displayed never parsed), a per-service summary read from the job's JSON state file (``services.read_state_json``: bffs bad-feed list + recent transitions, EOP table span, eigencal last transit; assembled in ``web._service_detail``), and an htmx-refreshed journal viewer (``job_logs``, one ``journalctl -u`` subprocess; ``?lines=`` clamped to 10–1000).  Everything is keyed off the ``web._service_registry()`` allowlist (``choco`` / ``eop`` / ``bffs`` / ``eigencal``) — page slugs are looked up there, never passed to journalctl raw.  Shared pill/colour macros live in ``_service_macros.html``.  Both monitors are instantiated unconditionally so the UI is uniform; if ``fpga_master.host/port`` (or ``psu.host/port``) are absent the badge renders as ``unconfigured`` and the greenlet doesn't spawn.  Failure handling is deliberate: monitor GETs get **one quick retry** (``_get_with_retry``) so a single dropped request doesn't flip a badge red for a whole 30 s interval; JSON of unexpected shape is tolerated (non-dict payloads ignored, ``_fetch_states`` raises ValueError so garbage and unreachable take the same error path); a PSU poll failure keeps the last-known grid with an explicit "showing stale states" banner on the page; and ``web._service_detail`` wraps all state-file summarising so corrupt job state degrades to "no summary", never a 500.  The ``fpga_master`` and ``psu`` blocks are **top-level** in ``config.yaml`` (``fpga_master`` was nested under ``eop`` historically); both ``app.load_config`` and ``jobs/eop/eop_update.py`` accept the legacy ``eop.fpga_master_host`` / ``eop.fpga_master_port`` keys but log a deprecation warning.  The ``bffs`` and ``eigencal`` config blocks (``service_unit``, ``state_file``) feed their badges.
