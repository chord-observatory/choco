...
- graphviz (`dot`) — optional; renders the pipeline graph on node status pages
...
The main page shows a table of all registered nodes with live-updating columns: node name, status, config, sync state, and Status/Edit links.
...
### Node Status (`/status/<group>/<node>`)

Click Status on a node (or "Live status" from its edit page) for the read-only live view — everything here is a kotekan read, so it works in maintenance mode too:
- **Buffers** — live table of kotekan's `/buffers` (polled every 5 s): per-buffer fullness (`3/24`), frame size, and a `held` badge for buffers with `peek_hold: true` in their kotekan config (kotekan keeps the newest frame around so it can always be peeked). Buffers with `peek_hold` get **Peek** and **Plot** buttons — a buffer without the hold usually has no full frame to serve, so buttons there would mostly error; enable `peek_hold: true` on a buffer's config block to make it watchable (the `/api/node-buffer-data` API itself accepts any frame buffer). **Peek** fetches the newest frame's descriptor and metadata (`len=0` — no data bytes, so it's cheap even on 400 MB frames); check the metadata timestamps for freshness. Fast-draining buffers without `peek_hold` may report "no full frame" — that's a timing miss, not an outage. An idle kotekan (process up, pipeline not running) has no buffer table at all and reports "No buffers reported".
- **Plot** (per frame buffer) — a live canvas view of the newest frame's data, refreshed every 5 s while the panel is on-screen and the tab visible. The renderer (`static/bufferplot.js`, dependency-free) fetches a bounded byte *prefix* through choco (`/api/node-buffer-data/…`, default 4 MiB, capped at 32 MiB) — the buffers are C-order with time leading, so a prefix is complete early timesamples with all inner structure intact (when one leading slice exceeds the fetch, the shape descends into the inner dimensions instead, so the axis selectors always have something to offer). It reads the frame descriptor to decode values (`float32`/`int32`/…, packed `int4x2` nibbles, `uint1x8` bits) and picks a view automatically: heatmap for multi-dimensional data (defaulting to the two largest axes), line plot when only one axis has any extent, value histogram when the frame has no descriptor extents. **Axes are composed**: the `y` and `x` controls are checkbox lists of the frame's dimensions, so a correlation frame can put `DPhi×DPlo1×DPlo2` on one axis and show all 9216 dish-pair combinations rather than averaging them; dimensions need not be adjacent. Any dimension left off both axes gets a **reduce or pin** control — reduce it (max/mean/min) or pin it to one index. Pinning is the important one for *categorical* dimensions: `C=2` is real/imaginary and `SK=3` is three separate statistics, so averaging across them mixes unlike quantities — pin `C=0` and you get the real part. Your axis layout and pins are remembered per buffer for the browsing session. **Boolean mask buffers** (`uint1x8`: the packet-loss and RFI masks) are handled specially, since kotekan's masks are **1 = good**: the packed bits are merged back into the axis they belong to (they are the fastest-varying continuation of the last dimension — for `pl_mask_exp`, time), the colour scale is pinned to 0…1 with amber for cleared bits against quiet slate for good ones, and the reduction defaults to **min**, so a cell lights up as soon as *any* sample under it is bad — a single lost packet is invisible under max or mean. The status line reports the set fraction (`100.00% set (1 = good)` is the healthy reading), and a `bits` selector switches between the merged axis and the raw 8-bit packing if you want to look at the bit layout itself. N2 frames are decoded into their kotekan frame-view blocks (vis, weight, flags, eigenvalues/vectors, gain, mask — selectable in the panel, with mag/phase/real/imag for complex blocks): vis and weight render as the full baseline matrix, and erms/χ² scalars appear in the status line. A **Meta** button in the panel fetches and shows the newest frame's metadata/descriptor JSON on demand (re-fetched fresh on each click). The status line shows the frame id and warns "no new frame since …" when the pipeline stops producing (with `peek_hold` the same held frame would otherwise be re-served silently). The watched buffer is remembered per node for the browser session, so revisiting the page reopens the live plot automatically; **Close** stops both the polling and the auto-reopen.
- **Pipeline Graph** — fetches kotekan's `/pipeline_dot` when the section scrolls into view and renders it with graphviz (curved edge routing), scaled to fit the page width; click the image to toggle full resolution in a scrollable view. The labels are a snapshot at fetch time — each buffer's array layout and fullness (`3/24 (12.5%)` = full frames / total frames — a high fraction means the consumer is falling behind), measured frame and byte rates, per-stage CPU — with a red outline on a buffer that has no free frame left and a dashed one on a buffer nothing has ever flowed through, so the refresh button re-fetches a fresh snapshot. The **Full-page view** link opens `/pipeline/<group>/<node>`, a standalone full-viewport page (slim header with CHOCO/status/edit links instead of the usual chrome; dark by default with a theme toggle — the whole graph is recolored via CSS on the inline SVG: fills go dark and outlines brighten, each node type keeping its hue, so blue buffers, amber CPU stages, orange GPU regions, violet I/O, magenta device memory and teal endpoints stay apart in either theme): the graph fills the screen (drag to pan, scroll to zoom around the cursor — trackpad pinch works too, shift+scroll stays a plain scroll — Fit/1:1 buttons, plus a live layout selector: curves (default)/ortho/polyline edge routing, an allowlisted `?layout=` preset that sticks in the URL, since crossing-heavy graphs read differently in each style), and buffer nodes with `peek_hold` carry a persistent amber outline marking them clickable — click anywhere in one (or tab to it and press Enter) and that buffer's live plot opens as a popup overlay in the corner, keeping the graph in view.  Zoom survives a refresh or a layout change, and if the buffer list can't be read the page says so rather than quietly showing a graph with nothing amber in it. The full-page SVG is inlined after a whitelist-reconstruction sanitizer, so kotekan-supplied content still can't run script.
