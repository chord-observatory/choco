...
- **Pipeline Graph** — fetches kotekan's `/pipeline_dot` when the section scrolls into view and renders it with graphviz (curved edge routing), scaled to fit the page width; click the image to toggle full resolution in a scrollable view. The labels are a snapshot at fetch time — each buffer's array layout and fullness (`3/24 (12.5%)` = full frames / total frames — a high fraction means the consumer is falling behind), measured frame and byte rates, per-stage CPU — with a red outline on a buffer that has no free frame left and a dashed one on a buffer nothing has ever flowed through, so the refresh button re-fetches a fresh snapshot. The **Full-page view** link opens `/pipeline/<group>/<node>`, a standalone full-viewport page (slim header with CHOCO/status/edit links instead of the usual chrome; dark by default with a theme toggle — graphviz's ink is recolored to light-on-dark via CSS on the inline SVG): the graph fills the screen (drag to pan, scroll to zoom around the cursor — trackpad pinch works too, shift+scroll stays a plain scroll — Fit/1:1 buttons, plus a live layout selector: curves (default)/ortho/polyline edge routing, an allowlisted `?layout=` preset that sticks in the URL, since crossing-heavy graphs read differently in each style), and buffer nodes with `peek_hold` carry a persistent amber outline marking them clickable — click anywhere in one (or tab to it and press Enter) and that buffer's live plot opens as a popup overlay in the corner, keeping the graph in view.  Zoom survives a refresh or a layout change, and if the buffer list can't be read the page says so rather than quietly showing a graph with nothing amber in it. The full-page SVG is inlined after a whitelist-reconstruction sanitizer, so kotekan-supplied content still can't run script.
